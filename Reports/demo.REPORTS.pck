...
CREATE OR REPLACE PACKAGE BODY REPORTS AS
  ...
  */
  ...
  BEGIN
    OPEN rfcReport FOR
      ...
      WHERE  s.service_id = srp.service_id
             ...
             AND i.utl_id = nvl(ipsUtlId, i.utl_id)
             AND s.commodity = nvl(ipsCommodity, s.commodity)
             AND s.utl_acct_id = nvl(ipsUTLAccountId, s.utl_acct_id)
             AND (s.service_id = ipnServiceid OR ipnServiceid = 0)
             AND (s.service_type IS NULL OR
             s.service_type = nvl(ipsServiceType, s.service_type))
    ...
    */
    ...
    OPEN rfcReport FOR
      SELECT s.customer_id "Customer ID",
             ...
             c.email "Email",
             trans.GET_STATUS(s.service_id) AS "Global Status"
      FROM   service s
      JOIN   customer c
      ON     s.customer_id = c.customer_id
      JOIN   utl_out_814_edi u
  ...
  BEGIN
    ...
    OPEN rfcReports FOR
      ...
      WHERE  1 = 1
             ...
             AND sv.service_type = nvl(ipsServiceType, sv.service_type)
             AND
  ...
  BEGIN
    ...
    OPEN rfcReport FOR
      SELECT u814.utl_short_name AS "Utility Name",
             ...
             round(AVG(nvl(i.energy_charges_amount, 0)), 4) AS "Average Energy Charges~~d4",
  ...
  BEGIN
    ...
    OPEN rfcReport FOR
      ...
      WHERE  1 = 1
             ...
             AND utl_id = nvl(ipsUtlId, utl_id)
             AND utl_acct_id = nvl(ipsUTLAccountId, utl_acct_id)
             AND (service_type IS NULL OR
             service_type = nvl(ipsServiceType, service_type))
    ...
    */
    ...
    BEGIN
      ...
      WHERE  usp.commodity = ipsCommodity
             ...
             AND month_year > to_date('31-dec-2012'); -- for now *** This needs tobe changed
    EXCEPTION
      WHEN OTHERS THEN
    ...
    FOR i IN 1 .. ipnUnbilledDays LOOP
      ...
      IF sCurrentIterationMonth <> sPriorPctMonth THEN
        ...
        WHERE  usp.commodity = ipsCommodity
               ...
               AND month_year > '31-dec-2012'; -- for now we more data there... 
        IF ipsServiceType = 'Residential' THEN
  ...
  BEGIN
    OPEN rfcReport FOR
      ...
      WHERE  1 = 1
             ...
             AND i820.utl_id = nvl(ipsUtlId, i820.utl_id)
      ...
      FROM   utl_rpm ur,
             (SELECT dtAsOfDate AS asOfDate,
              ...
              WHERE  1 = 1
                     ...
                     (s.service_type IS NULL OR
                     s.service_type = nvl(ipsServiceType, s.service_type))
                     AND i.utl_acct_id = nvl(ipsUTLAccountId, i.utl_acct_id)) t
      
      WHERE  ur.utl_id = t.utl_id
      GROUP  BY asOfDate, ur.utl_short_name, t.COMMODITY;
  END;
  ---------------------------------------------------------------------------------------------
  ...
  BEGIN
    OPEN rfcReport FOR
    /*THESE NEED TO BE REVIEWED TO REMOVE CANCEL TRANSACTIONS ETC.*/
    /*THESE NEED TO BE REVIEWED TO REMOVE CANCEL TRANSACTIONS ETC.*/
      ...
      WHERE  1 = 1
             ...
             AND transaction_purpose = '00' --safety net, not really needed anymore, but future?
             AND trunc(i.transaction_date) BETWEEN ipsdtBeginDate AND
             ipsdtEndDate
             AND i.utl_id = nvl(ipsUtlId, i.utl_id)
             AND i.commodity = nvl(ipsCommodity, i.commodity)
             AND (s.service_type IS NULL OR
             UPPER(s.service_type) =
             UPPER(nvl(ipsServiceType, s.service_type)))
             AND (sc.sales_company_id = ipnSalesCompanyId OR
             ipnSalesCompanyId = 0)
             AND i.utl_acct_id = nvl(ipsUTLAccountId, i.utl_acct_id)
             AND (i.service_id = ipnServiceid OR ipnServiceid = 0)
  ...
  BEGIN
    OPEN rfcReport FOR
    --confirm and compile full list of whats considered 'Active'
    --add usage info?
      SELECT --ss 2/03/2014
       u814.utl_short_name  AS "Utility Name",
       s.commodity          AS "Commodity",
      ...
      ON     sc.sales_company_id = ssrc.sales_company_id
      WHERE  1 = 1
             AND UPPER(trans.GET_STATUS(s.service_id)) IN
             ('ACTIVE', 'Active - Request Cancellation')
             AND s.commodity = nvl(ipscommodity, s.commodity)
             AND s.utl_id = nvl(ipsUtlId, s.utl_id)
             AND (s.service_type IS NULL OR
             UPPER(s.service_type) =
  ...
  BEGIN
    ...
    --add usage info?
      ...
      WHERE  1 = 1
             AND UPPER(trans.GET_STATUS(s.service_id)) IN
             ('ACTIVE', UPPER('Active - Request Cancellation'))
             AND s.commodity = nvl(ipscommodity, s.commodity)
             AND s.utl_id = nvl(ipsUtlId, s.utl_id)
             AND (s.service_type IS NULL OR
             UPPER(s.service_type) =
             ...
             AND (sc.sales_company_id = ipnSalesCompanyId OR
             ipnSalesCompanyId = 0)
      GROUP  BY u814.utl_short_name, s.commodity;
  END;
  ---------------------------------------------------------------------------------------------
  PROCEDURE ENR_EnrollmentRejectionsDetail(ipsdtBeginDate    IN VARCHAR2,
                                           ipsdtEndDate      IN VARCHAR2,
  ...
  BEGIN
    OPEN rfcReport FOR
      ...
      ON     s.utl_id = u.utl_id
             AND s.commodity = u.commodity
      WHERE  upper(trans.get_status(s.service_id)) LIKE '%REJECT%' --This is far from optimal and only meant as a placeholder
  ...
  BEGIN
    ...
    --Do we need to be left-joining to Customer/Service etc for conditions where no match can be made?
      ...
      WHERE  1 = 1
             ...
             AND se.utl_id = nvl(ipsUtlId, se.utl_id)
             AND (sc.sales_company_id = ipnSalesCompanyId OR
             ipnSalesCompanyId = 0)
             AND se.utl_acct_id = nvl(ipsUTLAccountId, se.utl_acct_id)
             AND (se.service_id = ipnServiceid OR ipnServiceid = 0)
             AND (se.service_type IS NULL OR
             se.service_type = nvl(ipsServiceType, se.service_type));
  END;
  ...
  BEGIN
    OPEN rfcReport FOR
      ...
      SELECT DISTINCT c.first_name AS "First Name",
                      ...
                      s.tpv_recording_id AS "TPV Recodring Id",
                      s.utl_acct_id AS "Utility Account ID",
                      u.utl_short_name AS "Utility Name",
                      s.commodity AS "Commodity",
                      s.service_id AS "Service ID",
      ...
      SELECT DISTINCT nvl(c.first_name, se.utl_rate_class) AS "First Name", -- FF we put it here FOR lack OF naywhere ELSE AS a customer was NOT created AND THE service escepytion TABLE does NOT have customer info
                      nvl(c.last_name, se.service_type) AS "Last Name", -- FF we put it here FOR lack OF naywhere ELSE AS a customer was NOT created AND THE service escepytion TABLE does NOT have customer info
                      nvl(c.street, se.street) AS "Address",
                      nvl(c.city, se.city) AS "City",
                      nvl(c.state, se.state) AS "State",
                      nvl(c.zip_code, se.zip_code) AS "Zip Code",
                      c.phone AS "Phone Number",
                      sc.sales_company_name AS "Sales Company",
                      se.sales_agent_id AS "Sales Agent Id",
                      se.tpv_recording_id AS "TPV Recodring Id",
                      se.utl_acct_id AS "Utility Account ID",
                      u.utl_short_name AS "Utility Name",
                      se.commodity AS "Commodity",
                      se.service_id AS "Service ID",
      ...
      WHERE  1 = 1 AND se.exc_description <> 'ReEnrollment' AND
       ...
       AND
       (se.utl_id IS NULL OR se.utl_id = nvl(ipsUtlId, se.utl_id)) --since missing UTL itself can cause exception
    ...
    OPEN rfcReport FOR
      ...
      FROM   (SELECT c.first_name AS "First Name",
              ...
              WHERE  1 = 1
                     ...
                     se.commodity = nvl(ipsCommodity, se.commodity)) --since missing commodity itself can cause exception
                     AND (se.utl_id IS NULL OR
                     se.utl_id = nvl(ipsUtlId, se.utl_id)) --since missing UTL itself can cause exception
                     AND
  ...
  END;
  ---------------------------------------------------------------------------------------------
  PROCEDURE EXC_ActiveButNoUsage
  ...
  BEGIN
    ...
    OPEN rfcReport FOR
      SELECT i.service_id AS "Service ID",
             c.first_name AS "First Name",
             c.last_name AS "Last Name",
  ...
  BEGIN
    ...
    OPEN rfcReport FOR
      SELECT DISTINCT 'Less Than Min' AS "Status",
      ...
      WHERE  rp.rate_plan_id = srp.rate_plan_id
             ...
             (SELECT MAX(end_date)
                  ...
                  WHERE  service_id = i.service_id
                         AND TRANSACTION_PURPOSE = '00')
             AND nvl(i.total_usage_amount, 0) <> 0
             AND ((i.Commodity = 'G' AND
             (round(nvl(i.rate_amount,
                              i.energy_charges_amount / i.total_usage_amount),
                          4) < se.rate_amount_min_gas)) OR
             (i.Commodity = 'E' AND
             (round(nvl(i.rate_amount,
                              i.energy_charges_amount / i.total_usage_amount),
                          4) < se.rate_amount_min_elec)))
      
      UNION
      
      SELECT DISTINCT 'More Than Max' AS "Status",
                      i.service_id AS "Service ID",
                      c.first_name AS "First Name",
                      c.last_name AS "Last Name",
                      DECODE(i.commodity,
      ...
      WHERE  rp.rate_plan_id = srp.rate_plan_id
             ...
             (round(nvl(i.rate_amount,
                              i.energy_charges_amount / i.total_usage_amount),
                          4) > se.rate_amount_max_gas)) OR
             (i.Commodity = 'E' AND
             (round(nvl(i.rate_amount,
                              i.energy_charges_amount / i.total_usage_amount),
                          4) > se.rate_amount_max_elec)))
  ...
  BEGIN
    ...
    OPEN rfcReport FOR
      ...
      FROM   (SELECT nvl(USAGE.GET_YEARLY_USAGE_AMOUNT(s.service_id), 0) AS YearlyUsagAmount,
                     ...
                     s.service_type AS "Service Type",
                     s.commodity AS "Commodity",
                     trans.GET_STATUS(s.service_id) AS "Status"
              
              
              FROM   service s, sales_source sr, utl_rpm uo, customer c
              WHERE  trans.GET_STATUS(s.service_id) NOT LIKE 'Cancelled%'
                     AND
                     trans.GET_STATUS(s.service_id) NOT LIKE '%Rejected%'
                     AND trans.GET_STATUS(s.service_id) NOT LIKE '%Rescind%'
                     AND uo.utl_id = s.utl_id
  ...
  BEGIN
    ...
    OPEN rfcReport FOR
      SELECT c.first_name AS "First Name",
             ...
             s.utl_acct_id AS "Utility Account ID",
             s.commodity AS "Commodity"
      FROM   service s, customer c, utl_out_814_edi uo, sales_source sr
  ...
  BEGIN
    OPEN rfcReport FOR
      SELECT c.Customer_id AS "Customer ID",
             ...
             sc.sales_company_name AS "Sales Company",
             s.service_type AS "Service Type",
             trans.GET_STATUS(s.service_id) AS "Service Status"
  ...
  BEGIN
    OPEN rfcReport FOR
      SELECT DISTINCT c.Customer_id AS "Customer ID",
                      ...
                      s.end_service_date AS "End Service Date",
                      sr.end_date - s.end_service_date AS "Days Difference",
                      (sr.end_date - s.end_service_date) * .83 AS "Early Termination Fee~~c2#",
                      100 AS "Flat Termination Fee~~c2#",
                      100 + ((sr.end_date - s.end_service_date) * .83) "Total Termination Fee~~c2#",
                      s.cancel_req_date AS "Cancel Request Date",
                      sc.sales_company_name AS "Sales Company",
      ...
      WHERE  s.end_service_date BETWEEN ipsdtBeginDate AND ipsdtEndDate
             ...
             AND s.cancel_req_date IS NOT NULL
             AND s.commodity = nvl(ipsCommodity, s.commodity)
             AND s.utl_id = nvl(ipsUtlId, s.utl_id)
             AND s.utl_acct_id = nvl(ipsUTLAccountId, s.utl_acct_id)
             AND (sc.sales_company_id = ipnSalesCompanyId OR
