CREATE OR REPLACE PACKAGE REPORTS AS
  ...
  */
  ...
  ----------------------------------------------------------------------------------------------- 
  FUNCTION get_All_energy_Adjustments(ipnInvoiceId           IN NUMBER,
                                      ipdFromTransactionDate IN invoice.transaction_date%TYPE)
    RETURN NUMBER;

  ------------------------------------------------------------------------------------------------  
  FUNCTION get_All_the_tax_Adjustments(ipnInvoiceId           IN NUMBER,
                                       ipdFromTransactionDate IN invoice.transaction_date%TYPE)
    RETURN NUMBER;

  -----------------------------------------------------------------------------------------------    
  FUNCTION get_All_energy_Adjustments_OH(ipnInvoiceId           IN NUMBER,
                                         ipnServiceId           IN service.service_id%TYPE,
                                         ipdFromTransactionDate IN invoice.transaction_date%TYPE)
    RETURN NUMBER;

  ----------------------------------------------------------------------------------------------------------------- 
  FUNCTION get_All_the_tax_Adjustments_OH(ipnInvoiceId           IN NUMBER,
                                          ipnServiceId           IN service.service_id%TYPE,
                                          ipdFromTransactionDate IN invoice.transaction_date%TYPE)
    RETURN NUMBER;
  ...
  */
  ...
  BEGIN
    OPEN rfcReport FOR
      ...
      SELECT DISTINCT c.first_name AS "First Name", -- FF 7/20/17 took out the nvl
  ...
  END;
  ------------------------------------------------------------------------------------------------
  --ak/ff nov/27/2017 the next 2 functions are to help get all the adjustments associated with a specific invoice to separate the energy and tax charges

  FUNCTION get_All_energy_Adjustments(ipnInvoiceId           IN NUMBER,
                                      ipdFromTransactionDate IN invoice.transaction_date%TYPE)
    RETURN NUMBER AS
  
    nTrInvoiceId             NUMBER := NULL;
    nEnergyChargesAmount     NUMBER := 0;
    nTempEnergyChargesAmount NUMBER := 0;
    nAdjustmentAmount        NUMBER;
    nInvoiceId               NUMBER;
  
    CURSOR cur IS
      SELECT f.triggering_invoice
      FROM   fin_trans f
      WHERE  invoice_id = nInvoiceId
             AND f.triggering_invoice IS NOT NULL;
  
  BEGIN
    nInvoiceId := ipnInvoiceId;
    LOOP
      FOR rec IN cur LOOP
        BEGIN
          SELECT i.energy_charges_amount, i.adjustment_amount
          INTO   nTempEnergyChargesAmount, nAdjustmentAmount
          FROM   invoice i
          WHERE  i.invoice_id = rec.triggering_invoice
                 AND i.transaction_date >= ipdFromTransactionDate;
        EXCEPTION
          WHEN OTHERS THEN
            nAdjustmentAmount        := NULL;
            nTempEnergyChargesAmount := NULL;
        END;
      
        nEnergyChargesAmount := nEnergyChargesAmount +
                                nvl(nTempEnergyChargesAmount, 0);
      
        IF nvl(nAdjustmentAmount, 0) <> 0 THEN
          nInvoiceid   := rec.triggering_invoice;
          nTrInvoiceId := nInvoiceId;
        ELSE
          ntrInvoiceId := NULL;
        END IF;
      ...
      EXIT WHEN nTrInvoiceId IS NULL;
    END LOOP;
    RETURN nEnergyChargesAmount;
  END;
  ------------------------------------------------------------------------------------------------
  FUNCTION get_All_the_tax_Adjustments(ipnInvoiceId           IN NUMBER,
                                       ipdFromTransactionDate IN invoice.transaction_date%TYPE)
    ...
    nTrInvoiceId             NUMBER := NULL;
    nStateSalesTAxamount     NUMBER := 0;
    nTempStateSalesTAxamount NUMBER := 0;
    nAdjustmentAmount        NUMBER;
    nInvoiceId               NUMBER;
    CURSOR cur IS
      SELECT f.triggering_invoice
  ...
  BEGIN
    ...
    LOOP
      FOR rec IN cur LOOP
        BEGIN
          SELECT i.state_sales_tax_amount, i.adjustment_amount
          INTO   nTempStateSalesTAxamount, nAdjustmentAmount
          FROM   invoice i
          WHERE  i.invoice_id = nvl(rec.triggering_invoice, 0)
                 AND i.transaction_date >= ipdFromTransactionDate;
        EXCEPTION
          WHEN OTHERS THEN
            nAdjustmentAmount        := NULL;
            nTempStateSalesTAxamount := NULL;
        END;
      
        nStateSalesTaxamount := nStateSalesTAxamount +
                                nvl(nTempStateSalesTAxamount, 0);
  ...
  FUNCTION get_All_energy_Adjustments_OH(ipnInvoiceId           IN NUMBER,
    ...
    nAdjustmentAmount        NUMBER;
    nInvoiceId               NUMBER;
    nTaxRate                 NUMBER := taxrateohio(ipnServiceId => ipnServiceId);
    CURSOR cur IS
      SELECT f.triggering_invoice
      FROM   fin_trans f
      WHERE  invoice_id = nInvoiceId
  ...
  BEGIN
    ...
    LOOP
      FOR rec IN cur LOOP
        BEGIN
          SELECT i.total_usage_amount *
                 (i.rate_amount - (i.rate_amount * nTaxRate)),
                 i.adjustment_amount
          INTO   nTempEnergyChargesAmount, nAdjustmentAmount
          FROM   invoice i
          WHERE  i.invoice_id = rec.triggering_invoice
  ...
  BEGIN
    ...
    LOOP
      FOR rec IN cur LOOP
        BEGIN
          SELECT i.total_usage_amount * nTaxRate, i.adjustment_amount
          INTO   nTempStateSalesTAxamount, nAdjustmentAmount
          FROM   invoice i
          WHERE  i.invoice_id = nvl(rec.triggering_invoice, 0)
                 AND i.transaction_date >= ipdFromTransactionDate;
