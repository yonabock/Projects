...
CREATE OR REPLACE PACKAGE BODY REPORTS AS
  ...
  YB     12/20/2017       GetListOfStates      Created new procedure to populate checkedComboBoxEdit for filtering by state
  
  YB     12/20/2017       Most Procedures      Added ipsStates and corresponding condition statement which accepts and parses
                                               a comma delimitted string.
                                               Below is example of method used to parse the comma delimitted string:
          rpm.state IN
             (SELECT nvl(regexp_substr(ipsStates, '[^,]+', 1, LEVEL),
                         rpm.state)
              FROM   dual
              CONNECT BY regexp_substr(ipsStates, '[^,]+', 1, LEVEL) IS NOT NULL) --selects into row 1 section of the string up until it hits
                                                                                  --a comma, then selects into row 2 next section
                                                                                  -- of the string until it hits a comma...
  
  YB     12/20/2017       Most Procedures      Updated the "*.utl_id = ipsUtlId" conditions to accept and parse a comma delimited
                                               list of utlIds. See previous entry for explenation of regex.
  */
