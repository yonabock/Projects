  ...
  */
  ...
  BEGIN
    ...
    LOOP
      ...
      dMonthYear     := dFetchedMonthYear; --the 
      ...
      LOOP
        ...
        IF dFetchedMonthYear = dMonthYear THEN
          ...
          --so we will go into the next loop and start processing it.
          --OTHERWISE we have no data for this dMonthYear so we have to calculate based on average.
        
          --The average-calculation was not put into an else clause, as one may have expected, because it might be possible
          ...
          LOOP
            ...
            IF nRemainderDays = 0 THEN
              ...
              END IF;
              IF dEndDate <= dStartDate THEN
                /* for a one-day-billing-cycle set end date to day after start date so that total days will =1
                because of nAdjustment it is possible that dEndDate became one day less than dStartDate
                therefor we check for <= as oppsed to just =, AND we set it to dStartDate +1 as opposed to dEndDate +1 */
              ...
              IF dEndDate > dLastOfMonth THEN
                nDaysWITHINCurMonthYear := dLastOfMonth - dStartDate; --number of days from CURRENT BILLING CYCLE that are in current month_year
                nRemainderDays          := nTotalDays -
                                           nDaysWithinCurMonthYear;
              ELSE
                nDaysWithinCurMonthYear := dEndDate - dStartDate;
              END IF;
            
            
              nAccumulatedAmount := nAccumulatedAmount + (nDaysWithinCurMonthYear *
                                    nAvgDailyUsage); -- calculated, (so far,) usage amount for current month_year
              nAccumulatedDays   := nAccumulatedDays +
                                    nDaysWITHINCurMonthYear;
            
            ELSE
              --we already aclculated some or all days for this month
              IF nRemainderDays >= nDaysInMonth THEN
                --we already calculated ALL days for this month
                nAccumulatedAmount := nDaysInMonth * nAvgDailyUsage;
                nRemainderDays     := nRemainderDays - nDaysInMonth;
              ELSE
                --we only calculated some days for this month
            ...
            IF nRemainderDays = 0 THEN
              ...
              /* 
              when dNewMonthYear (of new record) > dMonthYear then we are up to a new month so exit this loop (3),
              then insert calculated info, and then begin loop 2 for the new month.
              OR recHUMU.Humuservice_Id <> nServiceId then we are up to a new serviceId so exit this loop (3),
              then insert calculated info,
              then exit loop 2 as well to begin loop 1 again for the new serviceId
              */
              EXIT WHEN dFetchedMonthYear > dMonthYear OR recHUMU.service_Id <> nServiceId;
            END IF;
          END LOOP; --(loop 3) end of loop that loops through all billing cycles that overlap current yearMonth
          ...
          ELSE
            IF nAccumulatedAmount > 0 THEN
              ...
              nAvgDailyUsage       := nAccumulatedAmount / nAccumulatedDays;
              nAccumulatedAmount   := nDaysInMonth * nAvgDailyUsage;
            END IF;
          END IF;
        ...
        IF nAccumulatedDays = 0 THEN
          ...
          BEGIN
            ...
            WHERE  utl_id = recHUMU.utl_id ---
                   ...
                   AND
                   to_char(month_year, 'MON') = to_char(dMonthYear, 'MON')
                   AND rownum < 2
        ...
        EXIT WHEN recHUMU.service_Id <> nServiceId;
