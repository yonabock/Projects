  ...
  */
    ...
    CURSOR curHUMU IS
      ...
      FROM   (SELECT 'HU' SOURCE_TABLE,
              ...
              WHERE  mu.transaction_purpose = '00'
                     AND mu.utl_tracking_id NOT IN
                     (SELECT mu.cancelled_utl_tracking_id
                          FROM   inb_867_mu mu)) humu
      JOIN   service s
      ...
      ORDER  BY service_id, humu.end_date ASC, humu.SOURCE_TABLE DESC;
  ...
  BEGIN
    ...
    LOOP
      ...
      --DBMS_OUTPUT.Put_Line(recHUMU.Service_Id);
          ...
          IF dFetchedMonthYear = dMonthYear OR
            ...
            LOOP
              ...
              */
              ...
              WHILE recHUMU.Start_Date = dStartDate LOOP
                --loop 3a
                -- we do the fetch in a while loop so we only use uniqe start dates
                FETCH curHUMU
                  INTO recHUMU;
                EXIT WHEN curHUMU%NOTFOUND; --Exit loop 3a
              END LOOP;
              ...
              */
              ...
              --END IF;
              /*
                WHEN nRemainderDays > 0 then we finished calculating everything for the current monthYear
                SO exit this loop (3),
                THEN insert calculated info, and then begin loop 2 for the new month. 
              */
