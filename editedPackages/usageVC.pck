...
CREATE OR REPLACE PACKAGE BODY usage
  ...
  */
    ...
    */
    CURSOR curHUMU IS
      ...
      WHERE  (UPPER(humu.commodity) <> 'E' OR UPPER(humu.uom) = 'KH') --FOR ELECTRIC ACCEPT ONLY KH
             ...
             AND humu.end_date > humu.start_date
      ORDER  BY service_id, humu.start_date ASC, humu.SOURCE_TABLE DESC;
  
    recHUMU                 curHUMU%ROWTYPE;
    dStartDate              recHUMU.Start_Date%TYPE; --start date of the current billing period in HU/MU
    dEndDate                recHUMU.End_Date%TYPE; --end date of the current billing period in HU/MU
    nTotalDays              NUMBER; --total days of the current billing period in HU/MU
    nTotalUsage             recHUMU.total_usage_amount%TYPE; -- total usage for current billing period in HU/MU
    nAvgDailyUsage          NUMBER;
    dLastOfMonth            DATE; --calender dMonthYear
    nDaysWithinCurMonthYear NUMBER; --calender dMonthYear
    nDaysInMonth            NUMBER; --calender dMonthYear
  ...
  BEGIN
    ...
    dFetchedMonthYear := trunc(recHUMU.Start_Date, 'MON'); --this is set here once for the first record.
    --Subsequently it is set near the end of loop 3
    ...
    LOOP
      ...
      dMonthYear        := dFetchedMonthYear; --dMonthYear, dLastOfMonth, nDaysInMonth, dRemainderEndDate are set here once for each serviceId.
      dLastOfMonth      := last_day(dMonthYear); -- subsequently they are set near the end of loop 2.   
      nDaysInMonth      := dLastOfMonth - (dMonthYear - 1);
      dRemainderEndDate := dMonthYear - 1;
      ...
      BEGIN
        LOOP
          ...
          IF dFetchedMonthYear = dMonthYear OR
             dRemainderEndDate >= dMonthYear THEN
            --the most recent fetch is equal to the monthYear that we are up to, that means we have data for this dMonthYear,
            --OR dRemainderEndDate >= dMonthYear that means that we have remaining data from previous record relevant for this dMonthYear,
            --SO we will go into loop 3 and start processing it.
            --OTHERWISE we have no data for this dMonthYear so we have to calculate based on average, and we skip loop 3
            ...
            LOOP
              ...
              IF dRemainderEndDate >= dMonthYear THEN
                --we have at least some info remaining from last fetch for this dMonthYear, so let's procces it
                IF dRemainderEndDate >= dLastOfMonth THEN
                  --we have info for the whole month so we procces it and exit loop 3
                  nAccumulatedDays   := nDaysInMonth;
                  nAccumulatedAmount := nAccumulatedDays * nAvgDailyUsage;
                  EXIT; --exit loop 3
                ELSE
                  --the "remainder" info is only for part of the month
                  nAccumulatedDays   := dRemainderEndDate -
                                        (dMonthYear - 1);
                  nAccumulatedAmount := nAccumulatedDays * nAvgDailyUsage;
                END IF;
                dRemainderEndDate := dMonthYear - 1;
              ...
              /* values directly from current record of HU/MU */
              sHuOrMu    := recHUMU.Source_Table;
              dStartDate := recHUMU.Start_Date;
              dEndDate   := recHUMU.End_Date;
            
              /* 
              *set dEndDate one day earlier for those utilities that overlapp end date with next start date
              ...
              */
              ...
              nDaysWITHINCurMonthYear := LEAST(dEndDate, dLastOfMonth) -
                                         (dStartDate - 1); --number of days from CURRENT BILLING CYCLE
              --that are in current month_year
              ...
              IF dEndDate > dLastOfMonth THEN
                dRemainderEndDate := dEndDate;
              END IF;
            
              WHILE (recHUMU.Start_Date = dStartDate OR
                    recHUMU.End_Date - nAdjustment <= dRemainderEndDate) AND
                    recHUMU.Service_Id = nServiceID LOOP
                --loop 3a
                -- we do the fetch in a while loop so we only use uniqe start dates
                -- OR recHUMU.End_Date - adjustment = dRemainderEndDate EXPLANATION: sometimes even though we moved on to the next record,
                -- which follows the record from which we set the dRemainderEndDate, we can still have a case of recHUMU.End_Date - adjustment <= dRemainderEndDate
                -- because although we made sure to have a later start date it is still possible to be of a shorter time span.
                FETCH curHUMU
                  INTO recHUMU;
                EXIT WHEN curHUMU%NOTFOUND;
              END LOOP;
              EXIT WHEN curHUMU%NOTFOUND; --Exit loop 3
              dFetchedMonthYear := trunc(recHUMU.Start_Date, 'MON');
              /* 
                WHEN recHUMU.Humuservice_Id <> nServiceId then we are up to a new serviceId
                SO exit this loop (3),
                THEN insert calculated info,
                THEN exit loop 2 as well to begin loop 1 again for the new serviceId
                
                OR dNewMonthYear (of new record) > dMonthYear then we are up to a new month
                SO exit this loop (3),
                THEN insert calculated info, and then begin loop 2 for the new month.
                
                OR nRemainderDays >= dLastOfMonth then we finished calculating everything for the current monthYear
                SO exit this loop (3),
                THEN insert calculated info, and then begin loop 2 for the new month. 
              
              */
              EXIT WHEN dEndDate >= dLastOfMonth OR recHUMU.service_Id <> nServiceId OR dFetchedMonthYear > dMonthYear;
              --END IF;
            
              --EXIT WHEN nRemainderDays > 0;
          ...
          ELSE
            ...
            sHuOrMu              := 'AV';
            IF nAvgBillUsage IS NULL THEN
              sServiceType := nvl(recHUMU.service_type, sDefaultServiceType);
          ...
          BEGIN
            INSERT INTO test_service_monthly_usage
