    ...
    */
    CURSOR curHUMU IS
      ...
      FROM   (SELECT 'HU' SOURCE_TABLE,
                     ...
                     hu.uom humuUom
              FROM   inb_867_hu hu
              --WHERE  hu.start_date < hu.end_date -- ??? why this check & why only for hu and not mu?
      ...
      ON     humu.humuService_id = smu.service_id
            /*joins on humu date range overlapps smu month, ultimately we are selecting where smu.service_monthly_usage_id IS NULL,
            in other words, where there IS NO overlap */
             AND (humu.start_date <= last_day(smu.month_year) AND
             /*humu.end date overlapps humu.start date so we will use humu.end_date - 1 */
             humu.end_date - 1 >= smu.month_year)
      WHERE  (UPPER(humu.commodity) <> 'E' OR UPPER(humu.humuUom) = 'KH') --FOR ELECTRIC ACCEPT ONLY KH
             AND humu.humuUom IS NOT NULL
             AND humu.start_date IS NOT NULL
             AND humu.end_date IS NOT NULL
             AND smu.service_monthly_usage_id IS NULL;
  
    recHUMU        curHUMU%ROWTYPE;
    dHUMUstartDate recHUMU.Start_Date%TYPE;
  
    dHUMUendDate    recHUMU.End_Date%TYPE;
    nHUMUtotalUsage recHUMU.Total_Usage_Amount%TYPE;
  
  BEGIN
  
    sEnteredBy           := 'CREATE_MONTHLY_USAGE_YB_NEW';
    sSourceOfCalculation := 'COMPLETE_MONTHorPARTIAL_MONTHorFROM_AVERAGE';
    ...
    dHUMUstartDate  := recHUMU.Start_Date;
    dHUMUendDate    := recHUMU.End_Date;
    nHUMUtotalUsage := recHUMU.Total_Usage_Amount;
  
    ------------------
  
    SELECT SEQ_SERVICE_MONTHLY_USAGE_ID.Nextval
    INTO   nServiceMonthlyUsageID
    FROM   dual;
    ...
    VALUES
      (nServiceMonthlyUsageID,
       nServiceID,
       dMonthYear,
       ROUND(nUsageAmount, 5),
       sUnitOfMeasure,
       sHuOrMu,
       SYSTIMESTAMP,
       sEnteredBy,
